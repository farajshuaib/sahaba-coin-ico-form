...
const App: React.FC = () => {
  ...
  const submit = async (e: React.SyntheticEvent) => {
    ...
    // check if tokenFrom is not set, if not well set modal to select token...
    if (!tokenFrom) {
    ...
    // check if tokenFrom is not set, if not well set modal to select token...
    if (!priceFrom) {
      setLimitationAlert(true);
      return;
    }

    if (
      +priceFrom > environment.maxTokenAmount ||
      +priceFrom < environment.minTokenAmount
    ) {
      setLimitationAlert(true);
      return;
    }
  ...
  return (
    <div className="App">
      <section id="swapForm" className="relative overflow-hidden py-24 ">
        <img
          src={figure1}
          alt=""
        ...
        <div className="container mx-auto px-5 md:px-8 ">
          ...
          >
            ...
            <div>
              ...
              <div className="flex justify-between flex-wrap gap-4 mt-2">
                {pricePerEther && (
                  <Badge color="success">
                    price: 1 {environment.lockedToken.id} = {pricePerEther}{" "}
                    {tokenFrom.id}
                  </Badge>
                )}
          ...
          {active && (
            <button
              ...
              className="flex items-center gap-2 text-lg font-medium mx-auto bg-blue-500/20 p-3 rounded-lg text-primary mt-5 text-center uppercase leading-relaxed tracking-wide hover:bg-primary hover:text-white transition-all"
            >
              <img
